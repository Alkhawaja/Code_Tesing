...
x_vel = remove((x_acc)/2*samplePeriod)+x_vel;
y_vel = remove((y_acc)/2*samplePeriod)+y_vel;
z_vel = remove((z_acc)/2*samplePeriod)+z_vel;
...
int main(int argc, char **argv){
  ...
  ROS_INFO("Running...");
//Publishers
  pub_p = nh.advertise<geometry_msgs::Vector3Stamped>("/imu/position", 10);
